...
export function PreviewScreen({
  ...
  const [bundle, setBundle] = useState<ResumeBundle | undefined>(
    route.params.draft,
  );
  ...
  useFocusEffect(
    useCallback(() => {
      if (route.params.draft) setBundle(route.params.draft);
        ...
        resumeRepository
          .bundle(route.params.resumeId)
          .then(setBundle)
          .catch(error => Alert.alert('Could not load preview', String(error)));
  ...
  return (
    <SafeAreaView style={[styles.page, { backgroundColor: c.canvas }]}>
      <ScreenHeader
        ...
        subtitle={`${bundle.resume.paperSize} · ${
          templates.find(t => t.id === bundle.resume.templateId)?.name ?? 'CV'
        }`}
      ...
      >
        <View style={styles.paper}>
          ...
          ) : bundle.resume.templateId !== 'custom' ? (
            <GlobalResumePreview bundle={bundle} />
          ) : (
            <ReferenceResumePreview bundle={bundle} />
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
